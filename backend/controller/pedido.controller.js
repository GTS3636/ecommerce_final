...
const deletar = async (req, res) => {
    ...
    if (!codPedido) {
        return res.status(400).json({ error: "É preciso informar o código do pedido!" })
    }

    try {
        const pedidoExist = await Pedido.findByPk(codPedido)
        if (!pedidoExist) {
            return res.status(404).json({ error: "Não foi encontrado nenhum pedido com o código informado!" })
        }

        // Deletar o pedido
        await Pedido.destroy({ where: { codPedido: codPedido } })

        return res.status(200).json({ message: "Pedido deletado com sucesso!" })
    } catch (err) {
        console.error('Erro ao deletar o pedido:', err)
        return res.status(500).json({ error: 'Erro ao deletar o pedido. Tente novamente mais tarde.' })
    }
}

module.exports = { cadastrar, listar, atualizar, consultar, deletar }